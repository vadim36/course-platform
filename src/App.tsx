  ...
  contents: [
    {
      ...
      channelId: 1,
      title: "Math Course",
      description: "A deep knowledge",
    },
    {
      id: 2,
      channelId: 2,
      title: "C Course",
      description: "A perfect C Course",
    },
  ],
  contentData: [{ contentId: 2, body: "lorem50" }],
  premiumContentUsers: [{ userId: 3, channelId: 1 }],
  users: [{ id: 1, name: "Vadim", password: "123456" }],
};
...
function ContentChannel() {
  ...
  if (contentChannel.type === "close") {
    const premiumUser = db.premiumContentUsers.find(
      (u) => u.userId === user.id
    );
  ...
  return (
    <div>
      ...
      <ul>
        {db.contents
          .filter((content) => content.channelId === Number(id))
          .map((content) => {
            return (
...
function Modal({
  ...
  open,
  setOpen,
}: { open: boolean; setOpen: Dispatch<boolean> } & PropsWithChildren) {
  return (
...
function Content() {
  const { id } = useParams();
  const contentData = db.contentData.find(
    (data) => data.contentId === Number(id)
  );
...
function AppBar() {
  const [findedContentModalOpen, setFindedContentModalOpen] =
  ...
  return (
    <header className="header">
      ...
      <form
        onSubmit={(e) => {
          e.preventDefault();
          setFindedContentModalOpen(true);
        }}
      >
        <input
          type="text"
      ...
      <Modal open={findedContentModalOpen} setOpen={setFindedContentModalOpen}>
        <ul>
          {db.channels
            .filter((channel) => channel.name.includes(searchFilter))
            .map((channel) => (
          ...
          {db.contents
            .filter((content) => content.title.includes(searchFilter))
            .map((content) => (
              <Link to={`/content/${content.id}`}>
                <div className="card">
                  <h3>{content.title}</h3>
